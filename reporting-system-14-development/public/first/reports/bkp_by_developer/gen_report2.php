			 ...
			 if ($user_id=="Admin")
									  ...
									  if ($result){
										...
										while ($row = mysql_fetch_array($result)) {
											...
											while ((($x<13) && ($yr<$year)) || (($x<7) && ($yr==$year))) {
												...
												if (($cnt==0) || ($cnt=="0")) {
													...
													$query_new = "SELECT a.report_code as REPCODE, r.report_name as NAME FROM ami_all_reports as a, ami_reports as r WHERE a.report_code=r.report_code and a.branch_code='$bcode' and year='$yr' and month='$x' and (a.status > 0)  order by r.report_name"; 
													$result_new = @mysql_db_query($dbname,$query_new);
													$rep_str = '';
													while ($row_new = mysql_fetch_array($result_new)) {
														$rep_str .= $row_new['REPCODE'];
														$rep_str .= '<br>';
													}
								  ...
								  <tr bgcolor="#000000">
									<th colspan="15"><span class="pageheader"><font color="white">Reports received during <?echo $year-1; ?>-<? echo $year; ?></font></span></th>
								  </tr>
								  <tr  bgcolor="#000000">
									<td align="center"><span class="pageheader"><font color="white">Branch</font></span></td>
									<td align="center"><span class="pageheader"><font color="white">Jul</font></span></td>
			 ...
			 if ($user_id=="Admin")
									  ...
									  if ($result){
										...
										while ($row = mysql_fetch_array($result)) {
											...
											while ((($x<13) && ($yr<$year)) || (($x<7) && ($yr==$year))) {
												if ($status=="all") {
													$query1 = "SELECT count(*) as cnt FROM ami_all_reports where branch_code = '$bcode' and month='$x' and year='$yr'";
												} else {
									  ...
									  <tr bgcolor="#000000">
										<th colspan="25"><span class="pageheader"><font color="white">Reports received during <? echo $months["$month"]; ?><?echo $year-1; ?>-<? echo $year; ?></font></span></th>
									  </tr>
									 <?
									  //Get all branches
