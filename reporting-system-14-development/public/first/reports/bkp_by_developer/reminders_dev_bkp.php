...
if (($Error_Msg=='') && ($r_month!='')) {
	...
	{
		if ($exclude_users!="")
		$query = "select distinct b.* from ami_users a left join ami_users b on (a.branch_code = b.branch_code and b.user_type='P' and b.status='1') where b.user_email!='' ";
		if ($exclude_users!="")
...
//		print $theBranch.'['.$theRegion.']';
		...
		if ( (($theType!='P') || ($rOutstanding=='2')) && 
			...
			{
		print $theEmail. " <br>".  $subj."<br>".$message."<br>";
										...
										<tr>
											...
											<td>
											  <select name="r_level" size=1>
												 <option value="all" selected>All</option>
												<option <? if ($rLevel=="L") print selected; ?> value="L">Local</option>
												<option <? if ($rLevel=="I") print selected; ?> value="I">Imarat</option>
                                                                        <? if ($branch_code=="CA") { ?>
												<option <? if ($rLevel=="N") print selected; ?> value="N">National</option>
										<? } ?>
											 </select>
											  </td>
										  </tr>
